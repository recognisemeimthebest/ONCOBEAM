...
export default function PatientQueue({ patients, patientId, onSelect }) {
  return (
    <aside className="flex w-56 shrink-0 flex-col bg-navy text-white/90">
      {/* 헤더 */}
      <div className="flex items-center justify-between bg-navy-2 px-2.5 py-1.5">
        <span className="text-[12px] font-bold text-white">환자 목록</span>
        <span className="rounded-sm bg-white/15 px-1.5 py-0.5 text-[11px] tabular">
          {patients.length}명
        </span>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-1.5">
        {patients.length === 0 && (
          <p className="px-2 py-6 text-center text-[11px] text-white/50">해당 환자가 없습니다.</p>
        )}
        {patients.map((p) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5">
                <span className="truncate text-[12px] font-bold text-white tabular">{p.id}</span>
                <span className="ml-auto shrink-0 rounded-sm bg-white/15 px-1 text-[10px] text-white/80 tabular">
                  {p.sex}/{p.age}
                </span>
              </div>
              <div className="mt-0.5 text-[10.5px] text-white/60 tabular">{p.birth}</div>
